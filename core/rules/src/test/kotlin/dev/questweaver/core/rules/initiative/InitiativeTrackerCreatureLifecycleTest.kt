...
import dev.questweaver.core.rules.initiative.models.InitiativeEntryData
...
class InitiativeTrackerCreatureLifecycleTest : FunSpec({
    ...
    context("Adding creatures mid-combat") {
        test("add creature inserts at correct position based on initiative") {
            ...
            val initialOrder = listOf(
                ...
                InitiativeEntryData(creatureId = 3L, roll = 8, modifier = 0, total = 8)
            )
            
            val state = tracker.initialize(initialOrder)
            
            // Add creature with initiative 17 (should go between 1 and 2)
            val newEntry = InitiativeEntryData(creatureId = 4L, roll = 15, modifier = 2, total = 17)
            val newState = tracker.addCreature(state, newEntry)
        ...
        test("add creature at beginning of initiative order") {
            ...
            val initialOrder = listOf(
                ...
                InitiativeEntryData(creatureId = 2L, roll = 10, modifier = 1, total = 11)
            )
            
            val state = tracker.initialize(initialOrder)
            
            // Add creature with highest initiative
            val newEntry = InitiativeEntryData(creatureId = 3L, roll = 19, modifier = 3, total = 22)
            val newState = tracker.addCreature(state, newEntry)
        ...
        test("add creature at end of initiative order") {
            ...
            val newEntry = InitiativeEntryData(creatureId = 3L, roll = 5, modifier = 0, total = 5)
            val newState = tracker.addCreature(state, newEntry)
        ...
        test("add creature doesn't adjust turn index when inserted after current turn") {
            ...
            val newEntry = InitiativeEntryData(creatureId = 4L, roll = 9, modifier = 1, total = 10)
            val newState = tracker.addCreature(state, newEntry)
    ...
    context("Removing creatures from combat") {
        test("remove creature maintains turn order") {
            ...
            // Remove creature 2
            val newState = tracker.removeCreature(state, 2L)
            
            newState.initiativeOrder shouldHaveSize 3
            newState.initiativeOrder.map { it.creatureId } shouldBe listOf(1L, 3L, 4L)
            newState.initiativeOrder shouldNotContain InitiativeEntryData(2L, 15, 2, 17)
        }
        ...
        test("remove active creature advances turn") {
            ...
            val newState = tracker.removeCreature(state, 1L)
        ...
        test("remove creature after current turn doesn't adjust index") {
            ...
            val newState = tracker.removeCreature(state, 4L)
        ...
        test("remove last creature in order") {
            ...
            val newState = tracker.removeCreature(state, 3L)
    ...
    context("Complex lifecycle scenarios") {
        test("add and remove creatures in same combat") {
            ...
            // Add creature
            val newEntry = InitiativeEntryData(creatureId = 3L, roll = 15, modifier = 2, total = 17)
            state = tracker.addCreature(state, newEntry)
            state.initiativeOrder shouldHaveSize 3
            
            // Remove creature
            state = tracker.removeCreature(state, 2L)
        ...
        test("remove multiple creatures") {
            ...
            val initialOrder = listOf(
                ...
                InitiativeEntryData(creatureId = 5L, roll = 8, modifier = -1, total = 7)
            )
        ...
        test("remove all creatures except one") {
            ...
            state = tracker.removeCreature(state, 2L)
            state = tracker.removeCreature(state, 3L)
    ...
    context("Edge cases") {
        test("add creature to empty initiative order") {
            ...
            val emptyState = tracker.initialize(emptyList())
            
            val newEntry = InitiativeEntryData(creatureId = 1L, roll = 15, modifier = 2, total = 17)
            val newState = tracker.addCreature(emptyState, newEntry)
            
            newState.initiativeOrder shouldHaveSize 1
            newState.initiativeOrder[0].creatureId shouldBe 1L
        }
        ...
        test("add multiple creatures with same initiative") {
            ...
            val entry2 = InitiativeEntryData(creatureId = 2L, roll = 16, modifier = 1, total = 17)
            state = tracker.addCreature(state, entry2)
            
            val entry3 = InitiativeEntryData(creatureId = 3L, roll = 14, modifier = 3, total = 17)
            state = tracker.addCreature(state, entry3)
            
            state.initiativeOrder shouldHaveSize 3
            // Should be sorted by modifier tiebreaker
        }

        test("remove non-existent creature has no effect") {
            val tracker = InitiativeTracker()
            ...
            // Try to remove creature that doesn't exist
            val newState = tracker.removeCreature(state, 999L)
